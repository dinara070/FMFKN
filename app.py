...
# --- ПОЧАТКОВІ ДАНІ (Викладачі) ---
# Використовуємо st.session_state, щоб зміни зберігалися під час роботи програми
if 'teachers_data' not in st.session_state:
    st.session_state.teachers_data = {
        "Кафедра алгебри і методики навчання математики": [
            ...
            "Комарова Карина Вадимівна"
        ],
        "Кафедра математики та інформатики": [
            "Ковтонюк Мар'яна Михайлівна (Завідувач кафедри математики та інформатики)", 
            "Бак Сергій Миколайович (Заступник декана з наукової роботи)", 
        ...
        "Кафедра фізики і методики навчання фізики, астрономії": [
            "Сільвейстр Анатолій Миколайович (Завідувач кафедри)", 
            "Заболотний Володимир Федорович", 
            "Білюк Анатолій Іванович",
            "Думенко Вікторія Петрівна", 
            "Моклюк Микола Олексійович", 
            "Ксендзова Оксана Сергіївна", 
            "Мамічева Інна Олексіївна",
            "Мороз Ярослав Олексійович", 
            "Сіваєва Наталія Віталіївна", 
...
with tab_add:
    with st.form("add_teacher_form"):
        new_name = st.text_input("ПІБ викладача")
        target_dept = st.selectbox("Оберіть кафедру", list(st.session_state.teachers_data.keys()))
        submit_button = st.form_submit_button("Додати до списку")
        
        if submit_button and new_name:
            st.session_state.teachers_data[target_dept].append(new_name)
            st.success(f"Викладача {new_name} додано!")
            st.rerun()

with tab_import:
    st.info("Функція імпорту з CSV/Excel буде доступна незабаром.")

with tab_delete:
    st.warning("Оберіть викладача для видалення у списку нижче за допомогою іконки кошика.")

st.markdown("---")

# --- СПИСОК ВИКЛАДАЧІВ З КНОПКАМИ ДІЙ ---
for dept, teachers in st.session_state.teachers_data.items():
    with st.expander(f"📚 {dept}", expanded=True):
        # Кнопка швидкого додавання на початку кожної кафедри
        if st.button(f"➕ Додати співробітника на {dept}", key=f"btn_add_{dept}"):
            st.info("Скористайтеся формою 'Управління' вище.")
            
        for i, teacher in enumerate(teachers):
            col_name, col_edit, col_del = st.columns([0.8, 0.1, 0.1])
            
            with col_name:
                st.write(f"• {teacher}")
            
            with col_edit:
                if st.button("📝", key=f"edit_{dept}_{i}"):
                    st.toast(f"Редагування {teacher}...")
...
# --- СОРТУВАННЯ (Drag-and-Drop імітація) ---
st.sidebar.header("⚙️ Системні налаштування")
if st.sidebar.checkbox("Увімкнути режим сортування"):
    st.sidebar.write("У цьому режимі ви можете змінювати порядок викладачів за ієрархією.")
    # Тут можна інтегрувати бібліотеку streamlit-sortables для повноцінного drag-and-drop
